...
export default function JobsPage() {
  ...
  const filtered = useMemo(() => { const needle = query.trim().toLowerCase(); return jobs.filter((job) => (status === "all" || job.status === status) && (!needle || [job.id, job.job_type, job.shipment_id, job.error_code, job.safe_error].some((value) => value?.toLowerCase().includes(needle)))); }, [jobs, query, status]);
  return <div className="operations-page cf-integration-jobs-page">
    <PageHeader icon={Jobs} title="Processing jobs" description="Register job extraction, assessment, dan delivery yang dipersistenkan oleh workspace. Bukan stream raw application logs." actions={<ActionLink href="/observability" variant="secondary" icon={Pulse}>Buka Observability</ActionLink>} />
    ...
    <section className="cf-job-toolbar"><div className="cf-job-search"><Search size={16} /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari ID, jenis, shipment, atau error" aria-label="Cari processing job" /></div><AppSelect ariaLabel="Filter status job" value={status} onValueChange={setStatus} options={[{ value: "all", label: "Semua status" }, ...["QUEUED", "RUNNING", "SUCCEEDED", "FAILED", "DEAD_LETTER"].map((value) => ({ value, label: value }))]} /></section>
    <DataTableSurface title="Job register" description={`${filtered.length} dari ${jobs.length} job pada respons ini. Safe error ditampilkan hanya bila backend mencatat versi aman.`}>{result.isLoading ? <div className="cf-table-loading">Memuat processing job…</div> : result.error ? <EmptyState icon={<Jobs size={18} />} title="Job register tidak dapat dimuat" description={result.error instanceof Error ? result.error.message : "Coba muat ulang saat API tersedia."} /> : !filtered.length ? <EmptyState icon={<Jobs size={18} />} title="Tidak ada job yang cocok" description="Ubah kata kunci atau status untuk melihat job lain." /> : <div className="table-scroll"><Table><Table.Header sticky><Table.Row><Table.Head>Queued</Table.Head><Table.Head>Jenis</Table.Head><Table.Head>Status</Table.Head><Table.Head>Upaya</Table.Head><Table.Head>Prioritas</Table.Head><Table.Head>Shipment</Table.Head><Table.Head>Safe error</Table.Head></Table.Row></Table.Header><Table.Body>{filtered.map((job) => <Table.Row key={job.id}><Table.Cell><span className="cf-table-date">{date(job.queued_at)}</span></Table.Cell><Table.Cell><span className="table-cell-primary">{job.job_type}</span><br /><span className="cf-metadata mono">{job.id.slice(0, 8)}</span></Table.Cell><Table.Cell><OperationalState value={job.status} /></Table.Cell><Table.Cell><span className="mono">{job.attempts}/{job.max_attempts}</span></Table.Cell><Table.Cell>{job.priority}</Table.Cell><Table.Cell><span className="mono">{job.shipment_id?.slice(0, 8) || "—"}</span></Table.Cell><Table.Cell><span className="cf-safe-error">{job.safe_error || "—"}</span></Table.Cell></Table.Row>)}</Table.Body></Table></div>}</DataTableSurface>
  </div>;
}

...
const actorLabels: Record<string, string> = { user: "Human user", service_account: "Service account", system: "System" };
...
const metadataSummary = (metadata: Record<string, unknown>) => {
  ...
  return keys.length ? `${keys.length} metadata field${keys.length === 1 ? "" : "s"}` : "Tercatat";
};
...
function AuditDetailDialog({ event, onClose }: { event: AuditEvent; onClose: () => void }) {
  ...
  return <Dialog.Root open onOpenChange={(open) => { if (!open) onClose(); }}><Dialog className="cf-audit-dialog" size="lg">
    ...
    <dl className="cf-audit-detail-grid">
      <div><dt>Timestamp</dt><dd>{formatDate(event.created_at)}</dd></div><div><dt>Action</dt><dd>{readable(event.event_type)}</dd></div>
      <div><dt>Actor type</dt><dd><OperationalState value={event.actor_type || "system"} /></dd></div><div><dt>Identitas</dt><dd>{event.actor_display_name || actorLabels[event.actor_type] || "System"}</dd></div>
      <div><dt>Actor ID</dt><dd className="mono">{actorId || "—"}</dd></div><div><dt>Object</dt><dd>{event.entity_type}{event.entity_id ? ` · ${event.entity_id}` : ""}</dd></div>
      <div className="cf-audit-detail-grid__wide"><dt>Request correlation</dt><dd className="mono">{event.request_id || "Tidak tersedia untuk peristiwa ini"}</dd></div>
    </dl>
...
export default function AuditPage() {
  ...
  const eventOptions = useMemo(() => [{ value: "all", label: "Semua action" }, ...Array.from(new Set(events.map((event) => event.event_type))).sort().map((value) => ({ value, label: readable(value) }))], [events]);
  const entityOptions = useMemo(() => [{ value: "all", label: "Semua object" }, ...Array.from(new Set(events.map((event) => event.entity_type))).sort().map((value) => ({ value, label: value }))], [events]);
  ...
  return <div className="operations-page cf-audit-page">
    <PageHeader icon={Archive} title="Audit trail" description="Rekaman tenant-scoped untuk action, keputusan, akses, dan perubahan operasional. Gunakan filter untuk menyelidiki satu peristiwa." />
    <StateNotice title="Cakupan organisasi" tone="info">Hasil dibatasi pada workspace yang aktif. Tidak ada global user management atau peristiwa dari tenant lain di surface ini.</StateNotice>
    <section className="cf-audit-filter-bar" aria-label="Filter audit trail">
      ...
      <div className="cf-audit-selects"><AppSelect ariaLabel="Filter tipe actor" value={actorType} onValueChange={setActorType} options={[{ value: "all", label: "Semua actor" }, { value: "user", label: "Human user" }, { value: "service_account", label: "Service account" }, { value: "system", label: "System" }]} /><AppSelect ariaLabel="Filter action" value={eventType} onValueChange={setEventType} options={eventOptions} /><AppSelect ariaLabel="Filter object" value={entityType} onValueChange={setEntityType} options={entityOptions} /></div>
      <div className="cf-audit-search"><Search size={16} /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari actor, action, object ID, request ID" aria-label="Cari audit trail" /></div>
    </section>
